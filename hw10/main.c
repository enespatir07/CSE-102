...
void main(){
    ...
    if(towerSize>=STACK_BLOCK_SIZE){// if the tower size that user entered is larger than stackblocksize than realloc for stack arrays
        t1->array = realloc(t1->array,sizeof(int)*towerSize);
        t2->array = realloc(t2->array,sizeof(int)*towerSize);
        t3->array = realloc(t3->array,sizeof(int)*towerSize);
    }
...
stack *init_return(){
    ...
    s->array = (int*)calloc(s->maxsize,sizeof(int));// memory allocation for int array of stack s
    return s;
}
...
int push(stack * s, int d){
    ...
    s->array[s->currentsize] = d; // and at that index making an assignment
    return 1;
}

int pop(stack *s){
    return s->array[s->currentsize--]; // returns value at index s->currentsize and decrement currentsize by 1
}
void print_move(stack *startTower, char sname, stack *endTower, char ename)
{
    printf("\nMove disk %d from '%c' to '%c'",startTower->array[startTower->currentsize],sname,ename);
    push(endTower,startTower->array[startTower->currentsize]);
    pop(startTower);   
}
...
void movement(stack *s, char snm, stack *e, char enm, int modofcounter){   
    ...
    if(modofcounter==1){//Movement of disk 1
        if(s->array[s->currentsize]==1)//checking if disk 1 is at top of stack s
            print_move(s,snm,e,enm);
        else if(e->array[e->currentsize]==1) //checking if disk 1 is at top of stack e
            print_move(e, enm, s, snm);
    }
    else if(modofcounter==0){// Movement of other disks
        if(e->currentsize!=0 &&  e->array[e->currentsize] < s->array[s->currentsize])//When all poles have at least 1 disk placing small number over the greater one
            print_move(e, enm, s, snm);
        else if(e->currentsize==0) //If stack e doesnt have any disk place place disk at top of s to e
            print_move(s, snm, e, enm);
        else if(s->currentsize != 0 && s->array[s->currentsize] < e->array[e->currentsize])//When all poles have at least 1 disk placing small number over the greater one
            print_move(s, snm, e, enm);
