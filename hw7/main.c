...
void filltable(char sevenRandomChosenWords[][50])
{
    ...
    for (int i = 0; i < 7; i++)
    {
        ...
        // CHOSEN WORDS AND THEIR COORDINATES IN ORDER
        int newRow,newCol;
        int x = strlen(sevenRandomChosenWords[i]);
        if(yon==1){
        printf("%d.Coordinates: %d %d  Word: %s\n",i+1,r,c,sevenRandomChosenWords[i]);            
        }
        else
        {
            if(yon==0 && d==right){
            ...
            else if(yon==0 && d==down){
                newRow = x + r-1;
                newCol = c;
            }
            else if(yon==0 && d==diagonal){
                newRow = x + r-1;
                newCol = x + c-1;
            }
            printf("%d.Coordinates: %d %d  Word: %s\n",i+1,newRow,newCol,sevenRandomChosenWords[i]);            
        }
